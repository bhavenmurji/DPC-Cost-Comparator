#!/usr/bin/env npx tsx
/**
 * Fix Unknown States Script
 *
 * Parses city and state from the address field for providers
...
function parseAddressForState(address: string): { city: string; state: string } | null {
  // Skip non-parseable addresses
  if (
    !address ||
    address === 'Website Scraped' ||
    address === 'DPC Frontier' ||
    address.length < 10
  ) {
    return null
  }

  // Common pattern: "Street, City, State ZIP" or "Street, City, State  ZIP"
  // Also handles: "Street, City , State ZIP"

  // Try to find state name (full name)
  const lowerAddress = address.toLowerCase()
  for (const [stateName, abbrev] of Object.entries(stateMap)) {
    const statePattern = new RegExp(`,\\s*([^,]+),\\s*${stateName}\\s+\\d{5}`, 'i')
  ...
  // Try state abbreviation pattern: "City, ST ZIP"
  const abbrevPattern = /,\s*([^,]+),?\s+([A-Z]{2})\s+\d{5}/i
  const abbrevMatch = address.match(abbrevPattern)
  if (abbrevMatch) {
    return {
      city: abbrevMatch[1].trim().replace(/,\s*$/, ''),
      state: abbrevMatch[2].toUpperCase(),
    }
  }

  // Try pattern: "City, ST" at end
  const cityStatePattern = /,\s*([^,]+),?\s+([A-Z]{2})\s*$/i
  const cityStateMatch = address.match(cityStatePattern)
  if (cityStateMatch) {
    return {
      city: cityStateMatch[1].trim().replace(/,\s*$/, ''),
...
function getStateFromZip(zipCode: string): string | null {
  if (!zipCode || zipCode === '00000' || zipCode.length < 3) {
    return null
  }
...
async function main() {
  console.log('═══════════════════════════════════════════════════════')
  console.log('  Fix Unknown States')
  console.log('═══════════════════════════════════════════════════════\n')

  // Get all providers with unknown state
  const providers = await prisma.dPCProvider.findMany({
    where: { state: 'Unknown' },
    select: { id: true, name: true, address: true, city: true, state: true, zipCode: true },
  })

  console.log(`  Found ${providers.length} providers with unknown state\n`)

  let fixedByAddress = 0
  let fixedByZip = 0
  let unfixable = 0
  ...
  for (const provider of providers) {
    ...
    const parsed = parseAddressForState(provider.address)
    if (parsed) {
      newCity = parsed.city
      ...
      newState = getStateFromZip(provider.zipCode)
      if (newState) {
        fixedByZip++
      } else {
        unfixable++
        console.log(`  ✗ Could not fix: ${provider.name}`)
        console.log(`    Address: ${provider.address}`)
        console.log(`    Zip: ${provider.zipCode}`)
        continue
      }
    }
    ...
    await prisma.dPCProvider.update({
      ...
      data: {
        city: newCity || provider.city,
        state: newState,
      },
    })

    console.log(`  ✓ ${provider.name}: ${newCity || provider.city}, ${newState}`)
  }
  ...
  console.log(`  Fixed by address: ${fixedByAddress}`)
  console.log(`  Fixed by zip code: ${fixedByZip}`)
  console.log(`  Could not fix: ${unfixable}`)
  console.log('═══════════════════════════════════════════════════════\n')
